...
n_expert = 4    # @param {"min": 1, "max": 16} number of expert MLPs per layer
top_k = 2       # @param {"min": 1, "max": 16} experts used by each token
aux_loss_coef = 0.01 # @param {"min": 0, "max": 0.5, "step": 0.01} weight of the load-balancing loss
assert top_k <= n_expert, "top_k can not exceed n_expert"
...
for i in range(n_layer):
    ...
    state_dict[f'layer{i}.router'] = matrix(n_embd, n_expert) # scores each expert for each token
    for e in range(n_expert):
        state_dict[f'layer{i}.expert{e}.mlp_fc1'] = matrix(n_embd, 4 * n_embd)
        state_dict[f'layer{i}.expert{e}.mlp_fc2'] = matrix(4 * n_embd, n_embd)
params = list(state_dict.values()) # a handful of Tensors instead of thousands of Values
print(f"num params: {sum(p.data.size for p in params)} (each token only uses {top_k}/{n_expert} of the expert params)")
...
def moe(x, li): # x: (B, T, C) -> output (B, T, C), load-balancing loss
    router_probs = softmax(linear(x, state_dict[f'layer{li}.router'])) # (B, T, E)
    # route each token to its top_k experts. Picking is not differentiable, so the choice is a constant 0/1 mask;
    # gradients still reach the router through the probabilities of the experts that were picked.
    top = np.argsort(-router_probs.data, axis=-1)[..., :top_k]
    mask = np.zeros_like(router_probs.data)
    np.put_along_axis(mask, top, 1.0, axis=-1)
    gates = router_probs * mask
    gates = gates / gates.sum(axis=-1, keepdims=True) # renormalize over the chosen experts
    out = 0
    for e in range(n_expert):
        # for clarity every expert runs on every token and unchosen ones get gate 0.
        # real MoE kernels only send each token to its chosen experts, which is where the compute savings come from
        h = linear(x, state_dict[f'layer{li}.expert{e}.mlp_fc1']).relu()
        out = out + linear(h, state_dict[f'layer{li}.expert{e}.mlp_fc2']) * gates[..., e:e+1]
    # load balancing (Switch Transformer): n_expert * sum_e (fraction of tokens sent to e) * (mean router prob of e)
    # smallest when both are uniform, so the router learns to spread tokens over all experts
    frac = mask.reshape(-1, n_expert).mean(axis=0) / top_k
    mean_prob = router_probs.reshape(-1, n_expert).mean(axis=0)
    aux = (mean_prob * frac).sum() * n_expert
    return out, aux, frac
...
def gpt(idx): # idx: (B, T) array of token ids -> logits (B, T, vocab_size), aux loss, expert load
    B, T = idx.shape
    aux_total, loads = 0, []
    x = state_dict['wte'][idx] + state_dict['wpe'][np.arange(T)] # joint token and position embedding
    x = rmsnorm(x)
    ...
    for li in range(n_layer):
        ...
        x = x + x_residual
        # 2) Mixture-of-Experts block: replaces the single MLP
        x_residual = x
        x = rmsnorm(x)
        x, aux, frac = moe(x, li)
        aux_total, loads = aux_total + aux, loads + [frac]
        x = x + x_residual

    logits = linear(x, state_dict['lm_head'])
    return logits, aux_total / n_layer, loads # aux: 1.0 when perfectly balanced, up to n_expert when collapsed
...
for step in range(num_steps):
    ...
    logits, aux, loads = gpt(xb)
    logprobs = log_softmax(logits)
    lm_loss = -logprobs[np.arange(batch_size)[:, None], np.arange(block_size)[None, :], yb].sum() / yb.size
    loss = lm_loss + aux_loss_coef * aux
    ...
    busiest = max(frac.max() for frac in loads) # 1/n_expert when perfectly balanced, 1.0 when one expert gets it all
    print(f"step {step+1:4d} / {num_steps:4d} | loss {lm_loss.data:.4f} | aux {aux.data:.4f} | busiest {busiest:.3f} | lr {lr_t:.5f}")
...
for sample_idx in range(num_samples):
    ...
    for pos_id in range(block_size):
        logits = gpt(np.array([tokens]))[0].data[0, -1] / temperature # only the prediction at the last position matters

...
eps_adam = 1e-8
max_grad_norm = 1.0  # @param {"min": 0.01, "max": 100, "log": true} rescale the gradient whenever its total length exceeds this
...
for step in range(num_steps):
    ...
    # Gradient clipping: measure the length of the gradient of *all* parameters together, cap it at max_grad_norm
    grad_norm = sum((p.grad ** 2).sum() for p in params) ** 0.5
    if grad_norm > max_grad_norm:
        for p in params:
            p.grad = p.grad * (max_grad_norm / grad_norm) # same direction, shorter step
    ...
    print(f"step {step+1:4d} / {num_steps:4d} | loss {loss.data:.4f} | lr {lr_t:.5f} | grad_norm {grad_norm:.4f}")

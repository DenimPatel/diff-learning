...
eps_adam = 1e-8
# Let there also be Muon for the 2D hidden weight matrices; embeddings and the output head stay on Adam
muon_lr = 0.02       # @param {"min": 0.001, "max": 0.2, "log": true} peak step size for the Muon matrices
muon_momentum = 0.95 # @param {"min": 0, "max": 0.99} momentum of Muon
use_muon = [name.startswith('layer') for name in state_dict] # attention and MLP matrices
...
def newton_schulz(G, steps=5):
    # approximately orthogonalize G: keep its "directions" but set all its singular values to ~1,
    # so every direction of the update moves by a similar amount. Only matmuls, no SVD needed.
    a, b, c = 3.4445, -4.7750, 2.0315 # coefficients of an odd polynomial tuned to converge fast
...
for step in range(num_steps):
    ...
    for i, p in enumerate(params):
        if use_muon[i]: # Muon: momentum, then orthogonalize the update
            m[i] = muon_momentum * m[i] + p.grad
            update = newton_schulz(p.grad + muon_momentum * m[i]) # nesterov-style momentum
            scale = max(1, p.data.shape[1] / p.data.shape[0]) ** 0.5 # (nin, nout) storage: scale for tall/wide shapes
            p.data -= muon_lr * (1 - step / num_steps) * scale * update
            p.grad = 0
            continue

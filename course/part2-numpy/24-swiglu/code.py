...
mlp_hidden = 4 * n_embd * 2 // 3 # 3 matrices instead of 2: shrink the hidden size to keep the parameter count
...
for i in range(n_layer):
    ...
    state_dict[f'layer{i}.mlp_fc1'] = matrix(n_embd, mlp_hidden) # gate projection
    state_dict[f'layer{i}.mlp_fc3'] = matrix(n_embd, mlp_hidden) # up projection
    state_dict[f'layer{i}.mlp_fc2'] = matrix(mlp_hidden, n_embd) # down projection
...
def silu(x): # a.k.a. swish: x * sigmoid(x), a smooth ReLU
    return x * (1 + (-x).exp()) ** -1
...
def gpt(idx): # idx: (B, T) array of token ids -> logits (B, T, vocab_size)
    ...
    for li in range(n_layer):
        ...
        x_residual = x
        x = rmsnorm(x)
        gate = silu(linear(x, state_dict[f'layer{li}.mlp_fc1']))
        up = linear(x, state_dict[f'layer{li}.mlp_fc3'])
        x = linear(gate * up, state_dict[f'layer{li}.mlp_fc2']) # the gate decides, per hidden unit, how much passes
        x = x + x_residual

...
n_kv_head = 1   # @param {"options": [1, 2, 4]} key/value heads shared by groups of query heads (1 = MQA, n_head = MHA)
assert n_head % n_kv_head == 0, "n_head must be a multiple of n_kv_head"
...
for i in range(n_layer):
    ...
    state_dict[f'layer{i}.attn_wk'] = matrix(n_embd, n_kv_head * head_dim) # smaller: only n_kv_head heads
    state_dict[f'layer{i}.attn_wv'] = matrix(n_embd, n_kv_head * head_dim)
...
print(f"kv cache per token: {2 * n_layer * n_kv_head * head_dim} numbers (MHA would need {2 * n_layer * n_embd})")
...
def gpt(idx): # idx: (B, T) array of token ids -> logits (B, T, vocab_size)
    ...
    for li in range(n_layer):
        ...
        k = linear(x, state_dict[f'layer{li}.attn_wk']).reshape(B, T, n_kv_head, head_dim).transpose(0, 2, 1, 3) # (B, nkv, T, hd)
        v = linear(x, state_dict[f'layer{li}.attn_wv']).reshape(B, T, n_kv_head, head_dim).transpose(0, 2, 1, 3)
        kv_for_head = np.arange(n_head) // (n_head // n_kv_head) # e.g. 4 heads, 2 kv heads: [0, 0, 1, 1]
        k, v = k[:, kv_for_head], v[:, kv_for_head] # share each kv head with its group of query heads
